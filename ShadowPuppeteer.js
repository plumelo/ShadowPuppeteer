...
const { urlEquals } = require('codeceptjs/lib/assert/equal');
const { equals } = require('codeceptjs/lib/assert/equal');
const { empty } = require('codeceptjs/lib/assert/empty');
const { truth } = require('codeceptjs/lib/assert/truth');
const {
  xpathLocator,
  ucfirst,
  fileExists,
...
} = require('codeceptjs/lib/utils');
const {
  isColorProperty,
  convertColorToRGBA,
} = require('codeceptjs/lib/colorUtils');
const path = require('path');
const ElementNotFound = require('codeceptjs/lib/helper/errors/ElementNotFound');
const RemoteBrowserConnectionRefused = require('codeceptjs/lib/helper/errors/RemoteBrowserConnectionRefused');
const Popup = require('codeceptjs/lib/helper/extras/Popup');
 ...
 * ## Methods
 */
class ShadowPuppeteer extends Helper {
  constructor(config) {
    super(config);
  ...
  async waitNumberOfVisibleElements(locator, num, sec) {
    ...
    if (locator.isCSS()) {
      const visibleFn = function (locator, num) {
        const els = querySelectorShadowDom.querySelectorAllDeep(locator);
        if (!els || els.length === 0) {
          return false;
        }
  ...
  async waitForElement(locator, sec) {
    ...
    if (locator.isCSS()) {
      const visibleFn = function (locator, num) {
        ...
        return Array.prototype.filter.call(els, el => el.offsetParent !== null).length > 0;
      };
    ...
    if (context) {
      ...
      if (locator.isCSS()) {
        waiter = contextObject.waitForFunction((locator, text) => {
          const el = querySelectorShadowDom.querySelectorDeep(locator);
          if (!el) return false;
...
module.exports = ShadowPuppeteer;
...
async function $$deep(matcher, selector) {
	const arrayHandle = await matcher.executionContext().evaluateHandle(
			(element, selector) => querySelectorShadowDom.querySelectorAllDeep(selector, element),
			matcher, selector
	);
...
async function findElements(matcher, locator) {
  locator = new Locator(locator, 'css');
	if (!locator.isXPath()) return $$deep(matcher, locator.simplify())
